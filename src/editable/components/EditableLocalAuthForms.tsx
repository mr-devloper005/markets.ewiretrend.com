...
const inputClass = 'h-[3.25rem] rounded-full bg-[#f1f3f5] px-5 text-base font-bold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:ring-2 focus:ring-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-full bg-[var(--slot4-accent)] px-6 text-sm font-black text-white transition hover:bg-[var(--slot4-warm-accent)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[8px] px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
