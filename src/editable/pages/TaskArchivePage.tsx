    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[var(--slot4-page-text)]">
        <section className="bg-[var(--slot4-aqua)]">
          <div className="mx-auto flex max-w-[1180px] flex-col gap-6 px-4 py-12 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-16">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-warm-accent)]">Marketing Hub</p>
              <h1 className="mt-3 text-5xl font-black leading-tight sm:text-6xl lg:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md border-l-4 border-[var(--slot4-warm-accent)] pl-5 text-sm font-bold leading-7 text-[var(--slot4-muted-text)]">
              Timely perspectives, practical distribution ideas, and media-ready updates organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-[var(--slot4-page-text)]/10 bg-white">
          <div className="mx-auto flex max-w-[1180px] gap-3 overflow-x-auto px-4 py-5 text-sm font-black sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'rounded-full bg-[var(--slot4-accent)] px-4 py-2 text-white' : 'rounded-full bg-[#f1f3f5] px-4 py-2 hover:text-[var(--slot4-accent)]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'whitespace-nowrap rounded-full bg-[var(--slot4-accent)] px-4 py-2 text-white' : 'whitespace-nowrap rounded-full bg-[#f1f3f5] px-4 py-2 hover:text-[var(--slot4-accent)]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1180px] gap-6 bg-white px-4 py-10 sm:px-6 lg:grid-cols-[1.55fr_0.85fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden rounded-[8px] bg-black shadow-[0_24px_70px_rgba(6,31,68,.12)]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[var(--slot4-warm-accent)] px-4 py-2 text-[10px] font-black uppercase tracking-[0.16em]">{getCategory(lead, label)}</span>
            ...
            <div className="grid overflow-hidden rounded-[8px] bg-[var(--slot4-aqua)]">
              <div className="p-6 text-[var(--slot4-page-text)]">
                <p className="text-xs font-black uppercase tracking-[0.2em] text-[var(--slot4-warm-accent)]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight">What the hub is watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-t border-[var(--slot4-page-text)]/10 bg-white">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover transition group-hover:scale-105" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[var(--slot4-warm-accent)]">0{index + 1}</p>
        ...
        <section className="mx-auto max-w-[1180px] bg-white px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b border-[var(--slot4-page-text)]/12 pb-5">
            <h2 className="text-4xl font-black sm:text-5xl">More from the desk</h2>
            <form action={basePath} className="flex overflow-hidden rounded-full bg-[#f1f3f5]">
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-[8px] bg-white shadow-[0_12px_30px_rgba(6,31,68,.08)] transition hover:-translate-y-1">
                  <div className="aspect-[16/10] overflow-hidden bg-black">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.16em] text-[var(--slot4-warm-accent)]">
            ...
            <div className="rounded-[8px] border border-dashed border-[var(--slot4-page-text)]/25 bg-white p-12 text-center">
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-[var(--slot4-page-text)]/20 bg-white px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
            ...
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-[var(--slot4-page-text)]/20 bg-white px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
